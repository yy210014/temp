...
function GetCurrentCameraSetup()
	...
	CameraSetupSetField(
		...
		bj_RADTODEG * GetCameraField(CAMERA_FIELD_ANGLE_OF_ATTACK),
		duration
	)
	CameraSetupSetField(
		theCam,
		CAMERA_FIELD_FIELD_OF_VIEW,
		bj_RADTODEG * GetCameraField(CAMERA_FIELD_FIELD_OF_VIEW),
		duration
	)
...
function TerrainDeformationCraterBJ(duration, permanent, where, radius, depth)
	bj_lastCreatedTerrainDeformation =
		TerrainDeformCrater(GetLocationX(where), GetLocationY(where), radius, depth, R2I(duration * 1000), permanent)
...
function CreateImageBJ(file, size, where, zOffset, imageType)
	bj_lastCreatedImage =
		CreateImage(file, size, size, size, GetLocationX(where), GetLocationY(where), zOffset, 0, 0, 0, imageType)
...
function ReplaceUnitBJ(whichUnit, newUnitId, unitStateMethod)
	...
	if newUnitId == 1969713004 then
		newUnit =
			CreateBlightedGoldmine(GetOwningPlayer(oldUnit), GetUnitX(oldUnit), GetUnitY(oldUnit), GetUnitFacing(oldUnit))
	else
		newUnit =
			CreateUnit(GetOwningPlayer(oldUnit), newUnitId, GetUnitX(oldUnit), GetUnitY(oldUnit), GetUnitFacing(oldUnit))
	end
...
function CreateDestructableLoc(objectid, loc, facing, scale, variation)
	bj_lastCreatedDestructable =
		CreateDestructable(objectid, GetLocationX(loc), GetLocationY(loc), facing, scale, variation)
...
function CreateDeadDestructableLocBJ(objectid, loc, facing, scale, variation)
	bj_lastCreatedDestructable =
		CreateDeadDestructable(objectid, GetLocationX(loc), GetLocationY(loc), facing, scale, variation)
	return bj_lastCreatedDestructable
end
...
function MeleeDefeatDialogBJ(whichPlayer, leftGame)
	...
	if not bj_meleeGameOver and IsMapFlagSet(MAP_OBSERVERS_ON_DEATH) then
		DialogAddButton(
			d,
			GetLocalizedString("GAMEOVER_CONTINUE_OBSERVING"),
	...
	if bj_isSinglePlayer then
		...
		if GetGameDifficulty() ~= MAP_DIFFICULTY_EASY then
			...
			TriggerRegisterDialogButtonEvent(
				...
				DialogAddButton(
					d,
					GetLocalizedString("GAMEOVER_REDUCE_DIFFICULTY"),
...
function LeaderboardSetPlayerItemLabelColorBJ(whichPlayer, lb, red, green, blue, transparency)
	LeaderboardSetItemLabelColor(
		lb,
		LeaderboardGetPlayerIndex(lb, whichPlayer),
		PercentTo255(red),
		PercentTo255(green),
		PercentTo255(blue),
		PercentTo255(100.0 - transparency)
	...
	if IsPlayerInForce(GetLocalPlayer(), toForce) then
		...
		if whichUnit == nil then
			...
			DoTransmissionBasicsXYBJ(
				...
				GetPlayerColor(GetOwningPlayer(whichUnit)),
				GetUnitX(whichUnit),
				GetUnitY(whichUnit),
				soundHandle,
				unitName,
...
--
function TransmissionFromUnitTypeWithNameBJ(
	toForce,
	fromPlayer,
	unitId,
	unitName,
...
function MeleeStartingUnitsUndead(whichPlayer, startLoc, doHeroes, doCamera, doPreload)
	...
	if nearestMine ~= nil then
		...
		bj_ghoul[GetPlayerId(whichPlayer)] =
			CreateUnit(whichPlayer, 1969711215, ghoulX + 0.0 * unitSpacing, ghoulY + 0.0 * unitSpacing, bj_UNIT_FACING)
...
function MeleeStartingUnitsUnknownRace(whichPlayer, startLoc, doHeroes, doCamera, doPreload)
	...
	for _ in _loop_() do
		CreateUnit(
			whichPlayer,
			1853057125,
			GetLocationX(startLoc) + GetRandomReal(-256, 256),
			GetLocationY(startLoc) + GetRandomReal(-256, 256),
			GetRandomReal(0, 360)
		)
		index = index + 1
		if index == 12 then
...
function MeleeGetCrippledRevealedMessage(whichPlayer)
	return (GetLocalizedString("CRIPPLE_REVEALING_PREFIX") or "") ..
		(GetPlayerName(whichPlayer) or "") .. (GetLocalizedString("CRIPPLE_REVEALING_POSTFIX") or "")
end
...
function InitBlizzardGlobals()
	...
	for _ in _loop_() do
		...
		if
			GetPlayerController(Player(index)) == MAP_CONTROL_USER and
				GetPlayerSlotState(Player(index)) == PLAYER_SLOT_STATE_PLAYING
		 then
